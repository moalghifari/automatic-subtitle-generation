...
cuda_cmd="run.pl --gpu 2"

dev=data-fbank/test
train=data-fbank/train

dev_original=data/$test_folder
train_original=data/$train_folder
...
if [ $stage -le 1 ]; then
  ...
  $cuda_cmd $dir/log/train_nnet.log \
    steps/nnet/train.sh \
      ...
      ${train}_tr90 ${train}_cv10 data/$lm $ali $ali $dir || exit 1;
  # Decode,
  steps/nnet/decode.sh --nj 20 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt 0.1 \
    $gmm/graph_tgpr $dev $dir/decode_$test_folder || exit 1;
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/nnet/align.sh --nj 20 --cmd "$train_cmd" \
    $train data/$lm $dir ${dir}_ali || exit 1
fi
