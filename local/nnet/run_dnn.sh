...
if [ $stage -le 0 ]; then
  ...
  steps/nnet/make_fmllr_feats.sh --nj 10 --cmd "$train_cmd" \
     --transform-dir $gmmdir/decode_$test_folder \
     $dir data/$test_folder $gmmdir $dir/log $dir/data || exit 1
  # train
  dir=$data_fmllr/train
  steps/nnet/make_fmllr_feats.sh --nj 10 --cmd "$train_cmd" \
     --transform-dir ${gmmdir}_ali \
     $dir data/$train_folder $gmmdir $dir/log $dir/data || exit 1
...
if [ $stage -le 2 ]; then
  ...
  $cuda_cmd $dir/log/train_nnet.log \
    ...
    $data_fmllr/train_tr90 $data_fmllr/train_cv10 data/$lm $ali $ali $dir || exit 1;
  # Decode (reuse HCLG graph_tgpr)
  steps/nnet/decode.sh --nj 20 --cmd "$decode_cmd" --acwt 0.2 \
    $gmmdir/graph_tgpr $data_fmllr/test $dir/decode_$test_folder || exit 1;
fi
...
if [ $stage -le 3 ]; then
  ...
  steps/nnet/align.sh --nj 20 --cmd "$train_cmd" \
    $data_fmllr/train data/$lm $srcdir ${srcdir}_ali || exit 1;
  steps/nnet/make_denlats.sh --nj 20 --cmd "$decode_cmd" --acwt $acwt \
    --lattice-beam 10.0 --beam 18.0 \
    $data_fmllr/train data/$lm $srcdir ${srcdir}_denlats || exit 1;
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/nnet/train_mpe.sh --cmd "$cuda_cmd" --num-iters 6 --acwt $acwt \
    --do-smbr true \
    $data_fmllr/train data/$lm $srcdir ${srcdir}_ali ${srcdir}_denlats $dir || exit 1
  # Decode
  for ITER in 1 6; do
    steps/nnet/decode.sh --nj 20 --cmd "$decode_cmd" \
      --nnet ${dir}/${ITER}.nnet --acwt $acwt \
      $gmmdir/graph_tgpr $data_fmllr/test $dir/decode_it${ITER}_$test_folder || exit 1
  done 
fi
